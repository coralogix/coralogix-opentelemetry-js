...
import {selfDurationNsBySpanId} from "../../../src/trace/processors/self-duration";

export default describe('selfDurationNsBySpanId', () => {
    it('computes exclusive self-duration for a parent fully covered by one child (parent 0-100, child 20-80 -> 40/60)', () => {
        const result = selfDurationNsBySpanId([
        ...
        assert.strictEqual(result.get('parent'), BigInt(40), 'parent self-duration = 100 - (80-20) = 40');
        assert.strictEqual(result.get('child'), BigInt(60), 'leaf child self-duration = its own duration = 60');
    });

    it('a span with no children has self-duration equal to its own duration', () => {
        const result = selfDurationNsBySpanId([
    ...
    it('merges overlapping/concurrent children so covered time is not double counted', () => {
        ...
        // Merged coverage is [10,60) = 50, so self-duration = 100 - 50 = 50.
    ...
    it('never returns a negative self-duration', () => {
        const result = selfDurationNsBySpanId([
